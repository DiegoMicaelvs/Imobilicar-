...
var DatabaseStorage = class {
  ...
  async getRentals() {
    return await db.select({
      ...
      customerCpf: rentals.customerCpf,
      driverLicense: rentals.driverLicense,
      emergencyContact: rentals.emergencyContact,
      street: rentals.street,
      complement: rentals.complement,
      neighborhood: rentals.neighborhood,
      city: rentals.city,
      state: rentals.state,
      zipCode: rentals.zipCode,
      startDate: rentals.startDate,
      endDate: rentals.endDate,
      totalPrice: rentals.totalPrice,
      status: rentals.status,
      isNegativado: rentals.isNegativado,
      hasCheckin: rentals.hasCheckin,
      hasCheckout: rentals.hasCheckout,
      // Exclude heavy image arrays: checkInImages, checkOutImages
      checkInDate: rentals.checkInDate,
      checkOutDate: rentals.checkOutDate,
      checkInNotes: rentals.checkInNotes,
      checkOutNotes: rentals.checkOutNotes,
      checkinCompletedAt: rentals.checkinCompletedAt,
      contractGeneratedAt: rentals.contractGeneratedAt,
      contractUrl: rentals.contractUrl,
      paymentMethod: rentals.paymentMethod,
      paymentProofUrl: rentals.paymentProofUrl,
      paymentVerifiedAt: rentals.paymentVerifiedAt,
      checkoutCompletedAt: rentals.checkoutCompletedAt,
      checkpointTiresSame: rentals.checkpointTiresSame,
      checkpointFuelSame: rentals.checkpointFuelSame,
      checkpointHasDamages: rentals.checkpointHasDamages,
      checkpointDamagesNotes: rentals.checkpointDamagesNotes,
      checkpointRepairCost: rentals.checkpointRepairCost,
      repairPaid: rentals.repairPaid,
      finalizationDebtAmount: rentals.finalizationDebtAmount,
      finalizationPaymentMethod: rentals.finalizationPaymentMethod,
      finalizationContractUrl: rentals.finalizationContractUrl,
      finalizedAt: rentals.finalizedAt,
      // Exclude: selectedPlanIds (array)
      bonusDiscountUsed: rentals.bonusDiscountUsed,
      priceBeforeDiscount: rentals.priceBeforeDiscount,
      createdAt: rentals.createdAt,
      checkInImages: sql2`NULL`,
      checkOutImages: sql2`NULL`,
      selectedPlanIds: sql2`NULL`
    }).from(rentals).orderBy(desc(rentals.createdAt)).limit(200);
  }
  async getRental(id) {
...
var videoUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.mimetype.startsWith("video/")) {
      ...
      cb(new Error("Apenas arquivos de v\xEDdeo s\xE3o permitidos"));
    }
  }
});
async function registerRoutes(app2) {
  ...
  app2.post("/api/admin/users/:id/sales", async (req, res) => {
    try {
      ...
      user = await checkAndResetSales(user);
      ...
      if (vehicles2.length === 0) {
        ...
        if (adminUser && adminUser.cpf) {
          ...
          const matchingCustomer = customers2.find(
            (c) => c.cpf && c.cpf.replace(/\D/g, "") === adminUser.cpf.replace(/\D/g, "")
          );
          if (matchingCustomer) {
  ...
  app2.patch("/api/vehicles/:id", async (req, res) => {
    try {
      ...
      const vehicle = await storage.updateVehicle(req.params.id, validationResult.data);
      if (!vehicle) {
        return res.status(404).json({ error: "Vehicle not found" });
      }
      res.json(vehicle);
  ...
  app2.post("/api/rentals", async (req, res) => {
    try {
      ...
      if (validatedData.isMonthly && validatedData.startMonth && validatedData.endMonth) {
        rentalData.startDate = /* @__PURE__ */ new Date(validatedData.startMonth + "-01");
        const endDateParts = validatedData.endMonth.split("-");
  ...
  app2.post("/api/investor-with-vehicle", async (req, res) => {
    try {
      ...
      let investor = await storage.getInvestorByCpf(cpf);
      if (!investor) {
  ...
  app2.get("/api/customers", async (_req, res) => {
    try {
      const customers2 = await storage.getCustomers();
      const vehicles2 = await storage.getVehicles();
      const customersWithInvestorInfo = customers2.map((customer) => {
        const { password, ...sanitizedCustomer } = customer;
        return {
          ...sanitizedCustomer,
          hasInvestorVehicles: vehicles2.some((v) => v.ownerId === customer.id && v.isInvestorVehicle),
          hasPassword: !!password
          // Indica se o cliente tem senha cadastrada (para identificar cadastros públicos)
        };
      });
      ...
      if (validatedData.customerCpf) {
        ...
        if (existingCustomer) {
          ...
          const newCustomer = await storage.createCustomer({
            ...
            paymentDate: validatedData.customerPaymentDate,
...
async function setupVite(app2, server) {
  ...
  const vite = await createViteServer({
    ...
    configFile: false,
    // Não tentar carregar arquivo do disco
    customLogger: {
      ...viteLogger,
      error: (msg, options) => {
